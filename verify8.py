...
class  Welcome (BoxLayout):
    ...
    def start(self,dt):
        self.hashoov.text='\u2023[b][u][color=ff0000] In the movie, first you should film your group number on portrait orientation. [/color][/u][/b]'
        ...
        \n\u2022 [color=ff0000]Insert only movies in mp4 format.[/color] \
        ...
        \n\u2022 We recommend to keep movies in private usb and deleting them from the computer. '
...
class AntyCopy(BoxLayout):
        ...
        for f in os.listdir(path):
            if int(f[:2])<10:
                if f[2]!='.' or f[0]!='0' or not f[1].isdigit():
                    ...
                    Clock.schedule_once(self.wrong_submissions, 15)
            ...
            if (f.lower().endswith('avi')) or (f.lower().endswith('mp4')) or \
                ...
                self.fbwrite(f,created_os,start,created,processed,station,group,lab,year,semester)
                self.load_movie(pathRead,f,year,semester,lab,group)
    ...
    def load_movie(self,path,f,year,semester,lab,group):
        ds=storage.bucket()
        bob=ds.blob(f)
        bob.upload_from_filename(path)
        ds.rename_blob(bob,'Movies/{}/{}/{}/{}_{}'.format(year,semester,lab,group,f))


    def fbwrite(self,f,created_os,start,created,processed,station,group,lab,year,semester):
        # print(f,created_os,start,created,processed,station,group,lab)
        ref = db.reference('/{}/{}/{}/{}/'.format(year,semester,lab,group))
        ref.push({
            '{}'.format(f[:f.index('.')]):{
                'file':f,
